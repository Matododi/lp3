...
public class GUIListagemLivro extends JDialog {
    ...
    public GUIListagemLivro(List<Livro> texto) {
        ...
        String[] colunas = new String[]{"IdLivro", "Preco", "Caminho", "Descricao", "VendaIdVenda", "EditoraIdEditora", "GeneroIdGenero",};
        String[][] dados = new String[0][3];
        DefaultTableModel model = new DefaultTableModel(dados, colunas);
        JTable tabela = new JTable(model);
        scroll.setViewportView(tabela);
        for (int i = 0; i < texto.size(); i++) {
            String[] linha = new String[]{String.valueOf(texto.get(i).getIdLivro()), String.valueOf(texto.get(i).getPreco()), String.valueOf(texto.get(i).getCaminho()), String.valueOf(texto.get(i).getDescricao()), String.valueOf(texto.get(i).getVendaList()), String.valueOf(texto.get(i).getEditoraIdEditora()), String.valueOf(texto.get(i).getGeneroIdGenero()),};
            model.addRow(linha);
        }
        painelTa.add(scroll);
